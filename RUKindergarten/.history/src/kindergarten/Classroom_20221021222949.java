...
public class Classroom {
    ...
    public void deleteLeavingStudent (String firstName, String lastName) {
        ...
        if(studentsInLine != null){
            for(SNode pointer = studentsInLine; pointer.getNext() != null; pointer = pointer){
                SNode prev = pointer;
                pointer = pointer.getNext();
                int compared = pointer.getStudent().compareNameTo(newStudent);
                if((compared == 0)){
                    prev.setNext(pointer.getNext());
                }
            }
        } else if(musicalChairs != null){
            for(SNode pointer = musicalChairs.getNext(); pointer.getNext() != musicalChairs){
                SNode prev = pointer;
                pointer = pointer.getNext();
            ...
            for(int i=0; i<studentsSitting.length; i++){
                for(int j=0; j<studentsSitting[0].length; j++){
                    if(studentsSitting[i][j] != null){
                        if(studentsSitting[i][j].compareNameTo(newStudent) == 0){

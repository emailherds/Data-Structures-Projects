            ...
            if(size == 2){
                ...
                for(int i = 0; i<random+1; i++){
                    if(i == random){
                        ...
                        String a = musicalChairs.getNext().getStudent().getFullName();
                        sortByHeight(temp2);
                        musicalChairs.setNext(next); // sets next to two over
                    }
                    musicalChairs = musicalChairs.getNext();
                }
                SNode front1 = studentsInLine;
                String a = ""; 
                while(studentsInLine != null){
                    a+= studentsInLine.getStudent().getLastName()+" ";
                    studentsInLine = studentsInLine.getNext();
                }
                studentsInLine = front1;
            }
            size--;
            musicalChairs = temp;
    ...
    public void sortByHeight(SNode s){ // sorts children who got out of musical chairs by height in ascending order.
        ...
        while(!sorted){ 
            ...
            // last case: greater than compared node, and less than or equal to the next node.
                ...
                if(height == studentsInLine.getNext().getStudent().getHeight()){
                    SNode temp = studentsInLine.getNext().getNext();
                    studentsInLine.getNext().setNext(s);
                    s.setNext(temp);
                }
                else{
                    SNode temp = studentsInLine.getNext();

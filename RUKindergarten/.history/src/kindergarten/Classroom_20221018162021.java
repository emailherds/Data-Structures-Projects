...
public class Classroom {
    ...
    public void makeClassroom ( String filename ) {
        ...
        for(int i = 0; i < size-1; i++){
            ...
            Student add = std.getStudent();
            Student current = studentsInLine.getStudent();
            Student next = studentsInLine.getNext().getStudent();

            if(add.compareNameTo(next))
            while(add.compareNameTo(next)<0){
                if()
            }
        }
    }

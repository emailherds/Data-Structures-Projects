...
public class Classroom {
    ...
    public void playMusicalChairs() {
        ...
        while(size > 1){ //runs while the size of the circular list is not 1
            String bc = musicalChairs.getStudent().getFullName();
            String a = "";
            ...
            if(random == size-1){
                for(int i = 0; i < size; i++){
                    if(i == size-1){
                        a = musicalChairs.getNext().getStudent().getFullName();
                        String b = musicalChairs.getStudent().getFullName();
                        String c = musicalChairs.getNext().getNext().getStudent().getFullName();
                ...
                for(int i = 0; i<random+1; i++){
                    if(i == random){
                        ...
                        sortByHeight(new SNode(musicalChairs.getNext().getStudent(),null));
                        String ab = studentsInLine.getStudent().getFullName();
            ...
            musicalChairs = temp;
            String c = musicalChairs.getStudent().getFullName();
        }
        seatStudents();
        studentsInLine = null;
    } 

    private void sortByHeight(SNode s){ // sorts children who got out of musical chairs by height in ascending order.
        boolean sorted = false;
        ...
        if(studentsInLine != null){
            for(SNode pointer = studentsInLine; pointer != null; pointer = pointer.getNext()){
                if(pointer.getNext() == null){
                    pointer.setNext(new SNode(s, null));
                    pointer = pointer.getNext();
                }
            }
        }else if(musicalChairs != null){
            for(SNode pointer = musicalChairs.getNext(); pointer != musicalChairs; pointer = pointer.getNext()){
                if(pointer.getNext() == musicalChairs){
    ...
    public void deleteLeavingStudent(String firstName, String lastName) {
        ...
        if (studentsInLine != null) {
            String first = studentsInLine.getStudent().getFirstName().toLowerCase();
            String last = studentsInLine.getStudent().getLastName().toLowerCase();
            ...
            while (s.getNext() != null) {
                SNode previous = s;
                s = s.getNext();
                first = s.getStudent().getFirstName().toLowerCase();
                last = s.getStudent().getLastName().toLowerCase();
            ...
            String first = musicalChairs.getNext().getStudent().getFirstName().toLowerCase();
            String last = musicalChairs.getNext().getStudent().getLastName().toLowerCase();

            SNode s = musicalChairs.getNext();
            boolean case1 = false;
            while (s != musicalChairs && !case1) {
                first = s.getNext().getStudent().getFirstName().toLowerCase();
                last = s.getNext().getStudent().getLastName().toLowerCase();
    
                if (first.equals(firstName) && last.equals(lastName)) {
                    if(s.getNext() == musicalChairs){
                        case1 = true;
                        musicalChairs = s;
                        musicalChairs.setNext(s.getNext().getNext());
                    }else
            ...
            for (int i = 0; i < studentsSitting.length; i++) {
                for (int j = 0; j < studentsSitting[0].length; j++) {
                    if (studentsSitting[i][j] != null) {
                        if (studentsSitting[i][j].getFirstName().toLowerCase().equals(firstName) && studentsSitting[i][j].getLastName().toLowerCase().equals((lastName))) {
                            studentsSitting[i][j] = null;
                        }
                    }
                }

...
public class Classroom {
    ...
    public void makeClassroom ( String filename ) {
        ...
        for(int i = 0; i < size-1; i++){
            Student a = new Student(StdIn.readString(), StdIn.readString(), StdIn.readInt());
            SNode temp = new SNode(a, null);

            boolean hasSort = false;
            while(!hasSort){
                if(a.compareNameTo(first.getStudent()) < 0){
                    hasSort = true;
                    first = temp;
                    first.setNext(studentsInLine);
                }
                if(a.compareNameTo(studentsInLine.getStudent()) <= 0){
                    hasSort = true;

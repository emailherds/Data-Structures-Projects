...
public class Classroom {
    ...
    public void playMusicalChairs() {
        ...
        while(musicalChairs.getNext() != musicalChairs){ //runs while the size of the circular list is not 1
            int random = StdRandom.uniform(size);
            SNode temp = musicalChairs;
            for(int i = 0; i<random; i++){
                if(i == random - 1){
                    SNode next = musicalChairs.getNext().getNext();
                    sortByHeight(musicalChairs.getNext());
    ...
    private void sortByHeight(SNode s){ // sorts children who got out of musical chairs by height in ascending order.
        ...
        while(!sorted){ 
            ...
            else if(height > studentsInLine.getStudent().getHeight()&& height <= studentsInLine.getNext().get){
                SNode temp = studentsInLine.getNext();
                studentsInLine.setNext(std);
                std.setNext(temp);
                sorted = true;

...
public class LocateTitan {
    ...
    public static void main (String [] args) {
        ...
        for(int i = 0; i < num; i++){
            ...
            for(int j = 0; j < num; j ++){
                if(!dSet[i]&&mat[currentSource][j] != 0){
                    if(!dSet[j] && (minCost[currentSource] != Integer.MAX_VALUE) && (minCost[j] > minCost[currentSource]+mat[i][j])){
                        minCost[j] = minCost[currentSource]+mat[currentSource][j];
                    }
                }
            }

            StdOut.print(minCost[currentSource]+"\n\n");

...
public class GameOfLife {
    ...
    */
    ...
    public int numOfCommunities() {
        ...
        ArrayList<Integer> list = aliveCellList();
        ...
        uniqueIds.add(ids.get(0));

        for(int i = 1; i < ids.size(); i++){
            for(int j = 0; j < uniqueIds.size(); j++){
                if(ids.get(i) == uniqueIds.get(j)){
                    j = uniqueIds.size();
                }else if(j == uniqueIds.size()-1&&ids.get(i)!= uniqueIds.get(j)){
        ...
        return uniqueIds.size(); // update this line, provided so that code compiles
    }

    private ArrayList<Integer> aliveCellList(){
